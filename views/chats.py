...
# ==========================================
# 🚀 FRAGMENTOS DE RECARGA EN TIEMPO REAL
# ==========================================
# Try-Except para asegurar compatibilidad si el servidor de Streamlit tiene otra versión
try:
    run_bandeja = st.fragment(run_every=5) # Recarga la lista de chats cada 5s
    run_chat = st.fragment(run_every=3)    # Recarga los mensajes cada 3s
except AttributeError:
    run_bandeja = lambda f: f
    run_chat = lambda f: f

@run_bandeja
def render_columna_bandeja(telefono_actual, func_cambiar_chat, busqueda):
    try:
        ...
        with st.container(height=600):
            if df_clientes.empty:
                ...
                for _, row in df_clientes.iterrows():
                    ...
                    st.button(label, key=f"c_{t_row}", use_container_width=True, type=tipo, on_click=func_cambiar_chat, args=(t_row,))
    except Exception as e:
        st.error("Error cargando lista")

@run_chat
def render_columna_historial(telefono_actual):
    try:
        with engine.connect() as conn:
            tabla = get_table_name(conn)
            
            # 🔥 OPTIMIZACIÓN: Solo avisa a WAHA que leíste si hay mensajes pendientes
...
# ==========================================
# ESTRUCTURA PRINCIPAL (No se recarga sola)
# ==========================================
def render_chat():
    ...
    with col_lista:
        ...
        st.divider()
        # El buscador está FUERA del fragmento para que no interrumpa al escribir
        busqueda = st.text_input("🔍 Buscar:", placeholder="Nombre o teléfono...")
        render_columna_bandeja(telefono_actual, cambiar_chat, busqueda)

    # --- CHAT Y ENTRADA DE TEXTO ---
    with col_chat:
        if not telefono_actual:
            st.info("👈 Selecciona un chat de la izquierda.")
        else:
            # 1. HISTORIAL RECARGABLE (3 Segundos)
            render_columna_historial(telefono_actual)

            # 2. LOGICA ESTÁTICA PARA EL INPUT (Fuera del fragmento)
            ultima_sesion = None
            try:
                with engine.connect() as conn:
                    # Consulta ultra rápida solo para saber la última línea de este chat
                    res = conn.execute(text("SELECT session_name FROM mensajes WHERE telefono=:t AND session_name IS NOT NULL AND session_name != '' ORDER BY fecha DESC LIMIT 1"), {"t": telefono_actual}).fetchone()
                    if res and res[0]:
                        ultima_sesion = str(res[0]).strip().lower()
            except: pass
            ...
            # 3. ENTRADA DE TEXTO (No se interrumpe nunca)
            with st.form("chat_form", clear_on_submit=True):
                ...
                if btn and txt:
                    ...
                    if ok:
                        ...
                        st.rerun() # Fuerza una actualización de pantalla tras enviar
