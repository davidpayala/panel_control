...
def render_catalogo():
    ...
    with tab_gestion:
        ...
        if modo_catalogo == "🌱 Crear Nuevo":
            ...
            if "Medida Nueva" in tipo_creacion:
                ...
                with st.form("form_new_full"):
                    ...
                    if macro_sel == "Lentes": opciones_sub = ["Estilo Natural", "Estilo Fantasía", "Accesorios"]
                    else: opciones_sub = ["Peluca Natural", "Peluca Fantasía", "Accesorios Pelucas"]
        ...
        # ------------------------------------------------------------------------------
            ...
            if sku_edit:
                with engine.connect() as conn:
                    df_data = pd.read_sql(text("SELECT v.*, p.marca, p.modelo, p.nombre as nombre_prod, p.macro_categoria, p.categoria, p.diametro, p.color_principal, p.url_imagen, p.url_compra FROM Variantes v JOIN Productos p ON v.id_producto = p.id_producto WHERE v.sku = :sku"), conn, params={"sku": sku_edit})
                
                if not df_data.empty:
                    curr = df_data.iloc[0]
                    col_img, col_form = st.columns([1, 3])
                    with col_img:
                        if curr['url_imagen']: st.image(curr['url_imagen'], use_column_width=True)
                    ...
                    with col_form:
                        with st.form("form_edit_sku"):
                            ...
                            if new_macro == "Lentes": opts_sub = ["Estilo Natural", "Estilo Fantasía", "Accesorios"]
                            else: opts_sub = ["Peluca Natural", "Peluca Fantasía", "Accesorios Pelucas"]
                            ...
                            new_url_img = st.text_input("URL Imagen:", value=curr['url_imagen'] or "")
                            new_url_buy = st.text_input("URL Compra:", value=curr['url_compra'] or "")
                            ...
                            if st.form_submit_button("💾 Guardar Cambios"):
                                ...
                                try:
                                    with engine.connect() as conn:
                                        trans = conn.begin()
                                        conn.execute(text("UPDATE Variantes SET sku=:n_sku, medida=:n_med, precio=:n_pre, precio_rebajado=:n_prer WHERE sku=:old_sku"),
                                            {"n_sku": new_sku_val, "n_med": new_medida, "n_pre": new_precio, "n_prer": final_rebajado, "old_sku": curr['sku']})
                                        conn.execute(text("UPDATE Productos SET marca=:mar, modelo=:mod, nombre=:nom, macro_categoria=:macro, categoria=:cat, color_principal=:col, diametro=:dia, url_imagen=:uimg, url_compra=:ubuy WHERE id_producto=:idp"),
                                            {"mar": new_marca, "mod": new_modelo, "nom": new_nombre_prod, "macro": new_macro, "cat": new_cat, "col": new_color_prin, "dia": str(new_diametro), "uimg": new_url_img, "ubuy": new_url_buy, "idp": int(curr['id_producto'])})
                                        trans.commit()
        ...
        # ------------------------------------------------------------------------------
            ...
            # --- ANCLA DE MEMORIA PARA EL SELECTBOX ---
            ...
            if not df_padres.empty:
                # Generamos el label, pero guardamos el ID numérico
                ...
                # Buscamos en qué posición quedó el ID guardado en memoria
                idx_memoria = 0
                ...
                sel_padre_label = st.selectbox(
                    "Selecciona el Producto Padre a inspeccionar:", 
                    options=lista_titulos,
                    index=idx_memoria
                )
                
                # Actualizamos la memoria con lo que el usuario esté tocando ahora
                ...
                if not df_hijos.empty:
                    ...
                    with col_del_v:
                        with st.container(border=True):
                            ...
                            # --- SOLUCIÓN 1: MULTISELECT PARA BORRAR VARIAS A LA VEZ ---
                            skus_a_borrar = st.multiselect(
                                "Selecciona uno o varios SKUs para desintegrar:", 
                                options=df_hijos['sku'].tolist(),
                                placeholder="Puedes elegir varios..."
                            )
                            
                            if st.button(f"💥 Borrar {len(skus_a_borrar)} SKU(s) marcados", type="primary", disabled=len(skus_a_borrar)==0):
                                try:
                                    with engine.begin() as tx:
                                        for s_target in skus_a_borrar:
                                            tx.execute(text("DELETE FROM Variantes WHERE sku = :s"), {"s": s_target})
                                    st.success(f"¡Se eliminaron {len(skus_a_borrar)} variantes correctamente!")
                                    time.sleep(1)
                                    st.rerun() # Al recargar, la memoria del ID mantendrá la pantalla fija en este producto
                                except Exception as e:
                                    if "foreign key" in str(e).lower() or "integrityerror" in str(e).lower():
                                        st.error("❌ Rechazado: Al menos uno de los SKUs seleccionados ya está amarrado a boletas de venta pasadas. Pon su stock en 0 en su lugar.")
                        ...
                        with st.container(border=True):
                            ...
                            if st.button("💥 Desintegrar Raíz Completa", type="primary", disabled=not seguro_padre):
                                try:
                                    ...
                                    st.session_state['cat_padre_inspect_id'] = None # Como la raíz ya no existe, limpiamos su memoria
    ...
    with tab_marketing:
        ...
        if st.button("🚀 GENERAR FEED PRO", type="primary"):
            with st.spinner("Generando catálogo unificado..."):
                try:
                    with engine.connect() as conn:
                        df_raw = pd.read_sql(text("SELECT v.sku, p.macro_categoria, p.categoria, p.marca, p.modelo, p.nombre, p.url_imagen, v.precio, (v.stock_interno + v.stock_externo) as stock_total FROM Variantes v JOIN Productos p ON v.id_producto = p.id_producto WHERE p.url_imagen IS NOT NULL AND p.url_imagen != ''"), conn)
