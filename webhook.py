...
def home():
    return "Webhook Activo 🚀 v2.1 (Reply Fix)", 200
...
def recibir_mensaje():
    ...
    try:
        ...
        for evento in eventos:
            ...
            # --- 5. EXTRAER DATOS DEL REPLY (NUEVO) ---
            reply_id = None
            reply_content = None
            
            raw_reply = payload.get('replyTo')
            if isinstance(raw_reply, dict):
                # WAHA devuelve objeto completo {id:..., body:...}
                reply_id = raw_reply.get('id')
                reply_content = raw_reply.get('body')
            elif isinstance(raw_reply, str):
                # WAHA versión vieja devuelve solo ID string
                reply_id = raw_reply
            # ------------------------------------------
            ...
            try:
                ...
                with engine.connect() as conn:
                    ...
                    # Usamos ON CONFLICT por si WAHA reenvía el mismo mensaje
                    # Asumimos que la tabla tiene restricción unique en whatsapp_id o confiamos en el insert simple
                    conn.execute(text("""
                    ...
                    """), {
                        ...
                        "d": archivo_bytes,
                        "wid": payload.get('id'), # ID del mensaje actual
                        "rid": reply_id,          # ID del mensaje citado
                        "rbody": reply_content    # Texto del mensaje citado
                    })
                    
                    conn.commit()
                    print(f"✅ {telefono_corto}: Guardado con Reply OK.")
